...
echo ""
echo "🔗 Para usar no Container App (ACA: keyvaultref + secretRef):"
echo "  # 1. Adicionar secrets do Key Vault"
echo "  az containerapp update --name rag-overlabs-app --resource-group $RESOURCE_GROUP \\"
echo "    --set-secrets \\"
echo "      openai-api-key=keyvaultref:https://$KV_NAME.vault.azure.net/secrets/OpenAIApiKey \\"
echo "      audit-enc-key-b64=keyvaultref:https://$KV_NAME.vault.azure.net/secrets/AuditEncKey"
echo "  # 2. Referenciar nas env vars"
echo "  az containerapp update --name rag-overlabs-app --resource-group $RESOURCE_GROUP \\"
echo "    --set-env-vars \\"
echo "      OPENAI_API_KEY=secretref:openai-api-key \\"
echo "      AUDIT_ENC_KEY_B64=secretref:audit-enc-key-b64"
echo "  # NÃO use @Microsoft.KeyVault(...) em Container Apps; use keyVaultUrl + identity no YAML e secretRef nas env."
echo ""
