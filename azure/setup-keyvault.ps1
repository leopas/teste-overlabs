...
Write-Host ""
Write-Host "🔗 Para usar no Container App (ACA: keyvaultref + secretRef):" -ForegroundColor Cyan
Write-Host "  # 1. Adicionar secrets do Key Vault"
Write-Host "  az containerapp update --name rag-overlabs-app --resource-group $ResourceGroup \"
Write-Host "    --set-secrets \"
Write-Host "      openai-api-key=keyvaultref:https://$KvName.vault.azure.net/secrets/OpenAIApiKey \"
Write-Host "      audit-enc-key-b64=keyvaultref:https://$KvName.vault.azure.net/secrets/AuditEncKey"
Write-Host "  # 2. Referenciar nas env vars"
Write-Host "  az containerapp update --name rag-overlabs-app --resource-group $ResourceGroup \"
Write-Host "    --set-env-vars \"
Write-Host "      OPENAI_API_KEY=secretref:openai-api-key \"
Write-Host "      AUDIT_ENC_KEY_B64=secretref:audit-enc-key-b64"
Write-Host "  # NÃO use @Microsoft.KeyVault(...) em Container Apps; use keyVaultUrl + identity no YAML e secretRef nas env."
Write-Host ""
